...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (!user || !user.passwordHash) return res.status(401).json({ message: "Ungültige Anmeldedaten" });
...
app.post("/api/auth/register", async (req, res) => {
  ...
  if (INVITE_CODE && inviteCode !== INVITE_CODE) return res.status(403).json({ message: "Ungültiger Einladungscode" });
  if (!name || !email || !password) return res.status(400).json({ message: "Name, E-Mail und Passwort erforderlich" });
  const db = await getDb();
  if (!db) return res.status(503).json({ message: "Datenbank nicht verfügbar" });
  const existing = await db.select({ id: users.id }).from(users).where(eq(users.email, email.toLowerCase().trim())).limit(1);
  if (existing.length > 0) return res.status(409).json({ message: "E-Mail bereits registriert" });
  const passwordHash = await bcrypt.hash(password, 12);
  const result = await db.insert(users).values({ name: name.trim(), email: email.toLowerCase().trim(), passwordHash, role: "partner" });
...
app.get("/api/lina/content", async (req, res) => {
  ...
  res.json({
    ...
    posts: posts.map(p => ({
      id: p.id, text: p.caption, hook: p.hook, cta: p.cta, hashtags: p.hashtags,
      imageUrl: p.imageUrl ?? "", videoUrl: p.videoUrl ?? "",
      topic: p.title, contentType: p.contentType, platforms: p.platforms,
    })),
  });
});
...
app.get("/api/lina/library", async (req, res) => {
  ...
  const posts = await db.select({
    id: contentPosts.id, text: contentPosts.caption, hook: contentPosts.hook,
    cta: contentPosts.cta, hashtags: contentPosts.hashtags,
    imageUrl: contentPosts.imageUrl, videoUrl: contentPosts.videoUrl,
    topic: contentPosts.title, contentType: contentPosts.contentType,
  }).from(contentLibrary)
    .innerJoin(contentPosts, eq(contentLibrary.postId, contentPosts.id))
    .orderBy(desc(contentLibrary.id)).limit(limit);
...
app.get("/api/lina/products", async (req, res) => {
  ...
  const posts = await db.select().from(contentPosts)
    ...
    .orderBy(desc(contentPosts.createdAt)).limit(limit);
  res.json({ success: true, count: posts.length, products: posts.map(p => ({ id: p.id, name: p.title, description: p.caption, imageUrl: p.imageUrl ?? "" })) });
});

// ─── Make.com Callback (empfängt URLs zurück von Make Flows) ─────────────────

app.post("/api/make/callback", async (req, res) => {
  const { postId, imageUrl, videoUrl, thumbnailUrl, source } = req.body ?? {};
  ...
  if (Object.keys(updates).length > 0) {
    await db.update(contentPosts).set(updates).where(eq(contentPosts.id, Number(postId)));
    console.log("[Make Callback] Post " + postId + " updated:", updates, "source:", source ?? "unknown");
  }
  res.json({ success: true, postId, updated: Object.keys(updates) });
});
...
app.use("/trpc", createExpressMiddleware({
  ...
  createContext,
  onError: ({ error }) => {
    if (error.code !== "UNAUTHORIZED" && error.code !== "FORBIDDEN") console.error("[tRPC Error]", error);
  },
}));
