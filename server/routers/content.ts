...
// ─── Fallback: generate locally if no API key ────────────────────────────────
...
export const contentRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.status !== "all") {
        conditions.push(eq(contentPosts.status, input.status));
      }
  ...
  generate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.apiKey) {
        const prompt = `Erstelle einen viralen ${input.contentType} für LR Lifestyle Partner zum Thema: ${topicStr}. Plattformen: ${input.platforms.join(", ")}. Pillar: ${input.contentPillar}. Nische: ${input.niche ?? "Business & Lifestyle"}. Ton: ${input.tone ?? "motivating"}. Ziel: Interessenten gewinnen, viral gehen, Mehrwert liefern. Sprache: Deutsch. Sehr emotional, persönlich, mit starkem Hook.`;
        try {
          generated = await generateWithGoViral(input.apiKey, prompt, input.contentType);
        } catch {
      ...
      // Trigger Make Webhook 08: Visual Creator (async, non-blocking)
      const MAKE_VISUAL_WEBHOOK = "https://hook.eu2.make.com/ie5d6v5tmcs4bm9qlijccc8fgk1e1w18";
      const BASE_URL = process.env.BASE_URL ?? "https://sozialmedia.best";
      fetch(MAKE_VISUAL_WEBHOOK, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          contentPillar: input.contentPillar,
          platforms: input.platforms,
          callbackUrl: `${BASE_URL}/api/make/callback`,
        }),
      }).catch(() => {});
  ...
  submitForApproval: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(contentPosts)
        ...
        .where(and(
          eq(contentPosts.id, input.postId),
          eq(contentPosts.creatorId, ctx.user.id)
        ));
    ...
    .query(async ({ input }) => {
      ...
      return db
        .select({
          ...
          creatorId: contentPosts.creatorId,
          creatorName: users.name,
          creatorEmail: users.email,
        })
        .from(contentPosts)
        .innerJoin(users, eq(contentPosts.creatorId, users.id))
